...
export default async function OpsItAccessPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="w-full max-w-none space-y-5">
      ...
      {canManage ? (
        <details className="rounded-lg border border-primary-dark/10 bg-white" id="grant-create">
          ...
          <form action={createItAccessGrantAction} className="grid gap-4 border-t border-primary-dark/10 p-5 min-[520px]:grid-cols-2 lg:grid-cols-6">
      ...
      {grants.length === 0 ? (
        ...
        <ul className="space-y-3">
          {grants.map((grant) => (
            <li key={grant.id} className="rounded-2xl border border-primary-dark/10 bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary-dark/45">{grant.employee ? grant.employee.full_name : "Shared / unlinked"}</p>
                  <h2 className="mt-1 font-heading text-lg font-bold text-primary-dark">{grant.system_name}</h2>
                ...
                <span className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.1em] ${grant.status === "active" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-primary-dark/15 bg-primary-dark/[0.04] text-primary-dark/65"}`}>
