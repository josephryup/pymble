...
export default async function OpsItPoliciesPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="w-full max-w-none space-y-5">
      ...
      {canManage ? (
        <details className="rounded-lg border border-primary-dark/10 bg-white" id="policy-create">
          ...
          <form action={createItPolicyAction} className="grid gap-4 border-t border-primary-dark/10 p-5 min-[520px]:grid-cols-2 lg:grid-cols-6">
            ...
            <li key={policy.id} className="rounded-2xl border border-primary-dark/10 bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-primary-dark/55">{policy.ack_count} acknowledgement{policy.ack_count === 1 ? "" : "s"}</p>
                </div>
