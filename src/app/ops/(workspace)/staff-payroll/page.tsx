...
import {
  ...
  deleteStaffPayrollRunAction,
  editStaffPayrollItemAction,
...
function pageNotice(params: OpsSearchParams) {
  ...
  if (updated === "deleted") return { tone: "success" as const, message: "Unpaid staff payroll run deleted. Its advances are available for the next run." };
  if (updated === "advance_archived") return { tone: "success" as const, message: "Staff advance archived." };
  if (updated === "archived") return { tone: "success" as const, message: "Unpaid staff payroll run deleted. Its advances are available for the next run." };
  if (updated === "edited") return { tone: "success" as const, message: "Approved payroll item updated." };
              ...
              <div className="p-5" key={run.id}>
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Generated by {run.created_by_user?.full_name ?? "Unknown user"} · {formatDate(run.created_at)}
                    </p>
                  </div>
                  <div className="flex flex-wrap items-start gap-2">
                    ...
                    {canManage && run.status === "draft" ? (
                      <>
                        ...
                        <form action={deleteStaffPayrollRunAction}>
                          <input name="id" type="hidden" value={run.id} />
                          <OpsConfirmSubmitButton
                            className={OPS_DANGER_BUTTON_CLASS}
                    ...
                    {run.items.map((item) => (
                      ...
                      >
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                          {canManage && run.status === "approved" ? (
                            <form action={editStaffPayrollItemAction} className="flex items-center gap-1">
                              <input name="item_id" type="hidden" value={item.id} />
                              <input aria-label={`${item.full_name} basic pay`} className={`${OPS_INPUT_CLASS} w-24`} defaultValue={item.basic_pay} min="0" name="basic_pay" step="0.01" type="number" />
                              <input aria-label={`${item.full_name} housing allowance`} className={`${OPS_INPUT_CLASS} w-24`} defaultValue={item.housing_allowance} min="0" name="housing_allowance" step="0.01" type="number" />
                              <input aria-label={`${item.full_name} other allowances`} className={`${OPS_INPUT_CLASS} w-24`} defaultValue={item.other_allowances} min="0" name="other_allowances" step="0.01" type="number" />
                              <button className={OPS_SECONDARY_BUTTON_CLASS} title="Save payroll item" type="submit">
