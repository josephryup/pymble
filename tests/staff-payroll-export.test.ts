...
test("staff payroll export contains one register sheet with statutory totals", () => {
  const workbookBuffer = buildStaffPayrollExportXlsx(
  ...
  assert.equal(rows[0][0], "PYMBLE CONSTRUCTION — STAFF PAYROLL REGISTER");
  assert.equal(rows[5][1], 330);
  assert.equal(rows[10][7], 3_300);
  assert.equal(rows[10][8], 330);
  assert.equal(rows[10][9], 165);
  assert.equal(rows[10][15], 66);
});
