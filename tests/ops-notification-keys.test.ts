...
/** Every `idempotencyKey: <expr>` occurrence, with its file and line. */
function collectKeyExpressions() {
  ...
  for (const file of walk(OPS_LIB)) {
    ...
    lines.forEach((line, index) => {
      const match = line.match(/idempotencyKey:\s*(.+?),?\s*$/);
      if (!match) return;
      const expression = match[1].trim();
      // Skip type declarations and pass-throughs of a caller-supplied value.
      if (expression === "string;" || expression === "idempotencyKey,") return;
      found.push({
